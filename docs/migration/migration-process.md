...
Confluence, Notion, wikis, HTML help centers, SharePoint, spreadsheets, slide decks, support tickets, API specs — pick your migration's actual sprawl. Some notes on each, from having moved all of them at one point or another:
...
- **SharePoint and OneDrive** are libraries, not a format. Inventory what's actually in there — Word, PDF, spreadsheets, duplicates — before routing anything anywhere.
...
That seventh step is the one worth protecting. The rules you extract from your first five pages — how to name files, where images live, how tables get handled, which source formats always need manual review — become the playbook for the next five hundred. Skip it, and you'll relearn the same lessons on every batch that follows.
